...
async function aGem(param1, param2, param3) {
    if (global.captchaDetected) return;
    const filter = msg => msg.author.id === global.owoID  && msg.content.includes(global.displayName) && msg.content.match(/Inventory|Please wait |Please slow down~/)
    global.channel.sendTyping();
    ...
    switch (randomInt(0, 4)) {
        ...
        default:
            ...
            if(cmd.includes("hunt") && global.config.autoGem) {
                var filter = m => m.author.id == global.owoID && m.content.includes(global.displayName) && m.content.match(/hunt is empowered by| spent 5 .+ and caught a/)
                const collector = global.channel.createMessageCollector({filter, max: 1, time: 10_000})
